...
- Exact version lists per band, and exact job counts per event type (`pull_request` = lean, `workflow_dispatch` = full). Forge and NeoForge rows have no lean/full split, forward-JVM rows (#58) included.
